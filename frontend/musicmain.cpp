...
void MusicMain::resizeEvent(QResizeEvent *event) {
    ...
    playlistwidget->resizePlaylists(this->width());
    scrollWidget->setFixedWidth(this->width() * 0.8);
    // Обновляем размеры виджета при изменении размеров окна
    if (userProfile && this->width()>1340 && this->width()<1500) {
        userProfile->setFixedSize(this->width() * 0.4, 220);
        // playlistwidget->setFixedSize(this->width()*0.4, 400);
    }
    qDebug()<<this->width();
}
